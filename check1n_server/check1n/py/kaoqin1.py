...
def oneone(idnum,date2):
    ...
    for i in range(len(data)):
        if (data[i][0] == idnum and data[i][4] == date2) and (int(data[i][5]*60+data[i][6])-int(lastHour*60+lastMinute) > 15) and int(data[i][5])<20:
           mat.append(data[i])
           lastHour = data[i][5]
           lastMinute = data[i][6]
    if len(mat) > 3:
