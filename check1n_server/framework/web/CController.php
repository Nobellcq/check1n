...
class CController extends CBaseController
{
	 ...
	 *
	 * When a bootstrap is currently active, this method will call {@link CTheme::getViewFile} to determine
	 ...
	 * Otherwise, this method will return the corresponding view file based on the following criteria:
	 * <ul>
	 * <li>When a bootstrap is currently active, this method will call {@link CTheme::getLayoutFile} to determine
    ...
    public function renderWithLayoutData($view,$data=null,$layoutData=null,$return=false)
    {
        if($this->beforeRender($view))
        {
            $output=$this->renderPartial($view,$data,true);
            if(($layoutFile=$this->getLayoutFile($this->layout))!==false)
                $layoutDataNew = array('content' => $output);
                if($layoutData!=null){
                    $layoutDataNew = array_merge($layoutData,$layoutDataNew);
                }
                $output=$this->renderFile($layoutFile,$layoutDataNew,true);
