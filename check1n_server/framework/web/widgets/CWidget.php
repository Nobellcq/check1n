...
class CWidget extends CBaseController
{
	 ...
	 * @param boolean $checkTheme whether to check if the bootstrap contains a view path for the widget.
	 * @return string the directory containing the view files for this widget.
	 */
	public function getViewPath($checkTheme=false)
	{
		$className=get_class($this);
		$scope=$checkTheme?'bootstrap':'local';
		...
		else
		{
			if($checkTheme && ($theme=Yii::app()->getTheme())!==null)
			{
				...
				if(strpos($className,'\\')!==false) // namespaced class
					...
					$path.=$className;
				if(is_dir($path))
					return self::$_viewPaths[$className]['bootstrap']=$path;
			}
